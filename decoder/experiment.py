...
class WavTokenizer(VocosExp):
    ...
    def __init__(
        ...
        if self.resume:
            ...
            state_dict_fa_qa = dict()  # feature extractor encodec quantizer
            state_dict_fa_en = dict()  # feature extractor encodec encoder
            state_dict_fa_de = dict()  # feature extractor encodec decoder
            state_dict_bb = dict()  # backbone
            state_dict_hd = dict()  # head
            state_dict_mp = dict()  # multi-period discriminator
            state_dict_mr = dict()  # multi-resolution discriminator
            state_dict_dac = dict()  # DAC discriminator
    ...
    def on_train_start(self):
        ...
        for name, module in [
            ...
            ("head", self.head),
            ("multiperioddisc", self.multiperioddisc),
            ("multiresddisc", self.multiresddisc),
            ("dac", self.dac),
        ]:
            print(f"\n{name}:")
            print(module)
            ...
            print(f"参数大小约: {total_params * 4 / 1024 / 1024:.2f} MB (float32)")
